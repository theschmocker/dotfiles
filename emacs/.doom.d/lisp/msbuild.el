;;; msbuild.el --- Simple commands for building .NET Solutions/Projects with MSBuild -*- lexical-binding: t; -*-
...
;; Package-Requires: ((emacs "28.2") (f "0.20.0"))
    ...
    (with-temp-buffer
      ...
      (while (ignore-errors (re-search-forward msbuild--sln-project-regexp))
        (let* ((guid (match-string 1))
               (name (match-string 2))
               (path (match-string 3))
               (type (map-elt msbuild-project-type-guid-label-alist guid guid #'string=)))
          (push (make-msbuild-project :name name
                                      :type type
                                      :path path)
                projects))))
    (cons msbuild-project-all
          projects)))
...
(defun msbuild--normalize-path (path)
  "Naively normalize PATH for the current platform.

On Windows, replace forward slashes with back slashes.
Anywhere else, replaces back slashes with forward slashes."
  (let ((mismatched-path-sep (if (memq system-type '(cygwin windows-nt ms-dos))
                                 "/"
                               "\\")))
